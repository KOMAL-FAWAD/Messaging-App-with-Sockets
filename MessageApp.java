...
public class MessageApp {
    ...
    private final String senderName = "Komal";
    private final String senderNumber = "031025896336";
    private static int contactCount = 0;
    private int[] chatCount = new int[200];

    {
        preaddContact("Komal", "03221836655");
        preaddContact("Sadaf", "03239884714");
        preaddContact("Abeera", "03219348471");
        preaddContact("Iqra", "03218984692");
        preaddContact("Huma", "03008172665");
    }
    ...
    public String searchChats(String name) {
        int index = findContactIndexByName(name);
        if (index != -1) {
            StringBuilder chatHistory = new StringBuilder();
            chatHistory.append("========================================\n")
                    .append("           Chat with ").append(contacts[index].getName()).append("\n")
                    .append("========================================\n");

            Sms[] messages = Arrays.copyOf(chats[index], chatCount[index]);
            sortMessagesByTime(messages);
            for (Sms message : messages) {
                chatHistory.append(message.toString()).append("\n");
                message.markAsRead();
            }
            chatHistory.append("========================================");
            return chatHistory.toString();
        } else {
            return "No chats found with this contact.";
        }
    }
